...
export class MatricularAlumnoUseCase {
  ...
  async execute(colegioId: string, alumnoId: string, dto: MatricularAlumnoDto): Promise<Result<MatriculaResponseDto>> {
    ...
    const yaMatriculado = await this.matriculaRepository.existeMatriculaActiva(alumnoId, dto.añoAcademico);
