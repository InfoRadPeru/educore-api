...
function buildPrematricula(estado: 'PENDIENTE' | 'CONFIRMADA' | 'CANCELADA' = 'PENDIENTE'): Prematricula {
  return Prematricula.reconstitute({
    id: 'pre-1', colegioId: 'colegio-1',
    alumnoId: 'alumno-1', colegioNivelId: 'nivel-1',
    seccionId: null, añoAcademico: 2026,
...
describe('CancelarPrematriculaUseCase', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useCase = new CancelarPrematriculaUseCase(mockRepo);
  });

  it('cancela la prematricula y persiste las observaciones', async () => {
    ...
    const result = await useCase.execute('colegio-1', 'pre-1', 'El alumno cambió de opinión');

    expect(result.ok).toBe(true);
    expect(mockRepo.actualizar).toHaveBeenCalledWith('pre-1',
      expect.objectContaining({ estado: 'CANCELADA', observaciones: 'El alumno cambió de opinión' }),
    );
  });

  it('falla con CONFLICT cuando la prematricula ya fue confirmada', async () => {
  ...
  it('falla con CONFLICT cuando la prematricula ya fue cancelada', async () => {
    mockRepo.buscarPorId.mockResolvedValue(buildPrematricula('CANCELADA'));
  ...
  it('falla con NOT_FOUND cuando la prematricula no existe', async () => {
